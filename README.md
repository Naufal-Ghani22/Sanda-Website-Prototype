...
4. Perawatan → pilih pakaian, salin komposisi serat dari label, baca panduan → pilih repair, upcycle, atau donasi. Pada repair dan donasi, pilih area demo serta satu calon mitra contoh sebelum menyimpan rencana.
5. Aktivitas → lihat jejak empat langkah di atas, termasuk area dan mitra contoh yang dipilih, atau mulai tantangan 7 hari. Setiap hari berbeda dengan pemakaian tercatat menambah progres.
...
`check-demo.cjs` menguji perjalanan utama menggunakan Playwright pada server lokal. `check-material.cjs` menguji logika komposisi; `check-care-ui.cjs` menguji tampilan perawatan dan pengisian label. `check-plus.cjs` menguji logika padanan serta biaya per pemakaian; `check-plus-ui.cjs` menguji alur Plus dan lebar HP. `check-partners-ui.cjs` menguji pilihan area/mitra contoh, penyimpanan ke Aktivitas, dan tampilan mobile. Berkas `preview-*.png` menyimpan hasil pemeriksaan tampilan.
