...
(async () => {
  ...
  try {
    ...
    assert(await page.getByRole('heading', { name: 'Perbaiki yang rusak.' }).isVisible());
    assert(await page.getByRole('heading', { name: 'Teruskan lewat stasiun donasi.' }).isVisible());
    await page.locator('.care-paths').screenshot({ path: 'preview-care-paths-mobile.png' });
