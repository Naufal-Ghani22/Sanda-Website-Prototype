...
(async()=>{
 ...
 assert(await button('Pemakaian hari ini tercatat').isDisabled());
 ...
 await link('Cek belanja').click();await page.getByLabel('Nama pakaian').fill('Cardigan orange');await button('Bandingkan dengan lemari').click();assert(await page.getByText('Ada yang mirip di lemarimu.').isVisible());assert(await page.getByRole('link',{name:/Lihat perawatan/}).isVisible());
 await link('Perawatan').click();await button('Cari mitra perbaikan').click();await button('Cari mitra contoh').click();await page.locator('#modal').getByRole('button',{name:/Pilih Penjahit contoh A/}).click();await page.locator('#modal').getByRole('radio',{name:/Ganti kancing/}).check();await button('Tinjau pembayaran').click();await page.locator('#modal').getByRole('radio',{name:/QRIS demo/}).check();await button('Konfirmasi pembayaran demo').click();await page.keyboard.press('Escape');
 await link('Aktivitas').click();await page.getByText('Repair simulasi:',{exact:false}).waitFor();await button('Mulai tantangan hari ini').click();
 for(const width of [390,320,768,1440]) {await page.setViewportSize({width,height:900});for(const route of ['home','wardrobe','scan','outfit','shop','care','analytics']){await go(route);assert(!await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth+1),`${width}: ${route} overflow`);}if(width===390){await go('home');assert(await page.getByText('4 / 4 dicoba').isVisible());await page.locator('#mobile-nav').getByRole('button',{name:'Menu lainnya'}).click();await page.locator('#more-menu').getByRole('link',{name:'Perawatan'}).click();assert.equal(new URL(page.url()).hash,'#care');await go('home');await page.locator('#toast').evaluate(el=>el.classList.remove('show'));await page.screenshot({path:'preview-mobile.png',fullPage:true});}}
 ...
 console.log('PASS: upload, persistence, search, empty results, outfit, wear deduplication, purchase comparison, repair payment demo, challenge, invalid file, sample, blank name, reset, Escape; 28 responsive checks; no page errors.');
